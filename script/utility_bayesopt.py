# Created on 25 Jan 2023 by Zihao Wang
...
def sampling(params, n_sample, solvent_list, sampling_method):
    ...
    xlimits = [xlimit if xtype == FLOAT else [xlimit[0], xlimit[1]] for (xlimit, xtype) in
               zip(xlimits, xtypes)]
    ...
    if considerSolvent:
        x_pro = xt[:, 1:]
        x_sol = [random.choice(np.arange(0, s_ub + 1)) for _ in range(n_sample)]
        solvent_alias = [solvent_list[i] for i in np.array(x_sol, dtype=int)]
    else:
        solvent_alias = [params["solvent"]] * n_sample
        x_pro = xt
    x_pro = np.round(x_pro, 4)
...
class BOConvergence:
    def __init__(self, es_patience=10, bo_patience=10):
