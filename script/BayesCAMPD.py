# Created on 25 May 2023 by Zihao Wang
...
def main(params, obj="MIN", constr=("DIST_C4H8_T1", "DIST_C4H6_T2")):
    ...
    runBayesCAMPD = params["runBayesCAMPD"]
    extra_sample_num = params["extra_sample_num"]
    early_stop_patience = params["early_stop_patience"]
    random_seed = params["random_seed"]
    sampling_method, optim_method = params["sampling_method"], params["optim_method"]
    ...
    for n_sample in active_list_n_sample:
        ...
        if runBayesCAMPD:
            ...
            bo_converge = BOConvergence(bo_patience=early_stop_patience)
            ...
            while idx_iter < n_iter:
                ...
                if idx_iter == 1:
                    ...
                    if addNext:
                        ...
                        while x_tr.shape[0] < n_tr_ + extra_sample_num:
                ...
                if optim_method == "DE":
                    ...
                    class OptimProblem(ElementwiseProblem):
                        ...
                        def _evaluate(self, x_in, out, *args, **kwargs):
                            x_ = np.array([x_in[a] for a in x_name])
                            x_ = x_.reshape(1, -1)
                            g1hat = model_g1.predict(x_)
                            g2hat = model_g2.predict(x_)
                            out["F"] = acquisition_func(x_, params, model_y, y_tr_best)
                            out["G"] = np.column_stack([puri_spec_ - g1hat, puri_spec_ - g2hat])
                ...
                for idx_trial in range(params["n_trial"]):
                    ...
                    if optim_method == "DE":
                        ...
                        variables = {a: Real(bounds=b) if c == 0 else Integer(bounds=b)
                                     for a, b, c in zip(x_name, bound, integrality)}
                        problem = OptimProblem(vars=variables, n_obj=1, n_ieq_constr=2)
                        result = minimize(problem, algorithm, seed=seed_)
                        trailSuccess = False if result.X is None else True
                    ...
                    if trailSuccess:
                        if optim_method == "DE":
                            x_optim, fun_optim = result.x, result.fun
                        else:
                            x_optim, fun_optim = [result.X[a] for a in x_name], result.F.item()
                        candidate.append(x_optim)
                        next_af = None
            ...
            print(f"* optimal solution: {bo_converge.best}, {bo_converge.optimum}")
...
if __name__ == "__main__":
    ...
    main(get_params("QH", "OneShot", considerSolvent=True, saveInitialData=True, runBayesCAMPD=False))
    time.sleep(10)
    main(get_params("QH", "Bayesian", considerSolvent=True, saveInitialData=False, runBayesCAMPD=True))
    time.sleep(10)
    main(get_params("TAC", "Bayesian", considerSolvent=True, saveInitialData=False, runBayesCAMPD=True))
    time.sleep(10)
    main(get_params("TAC", "OneShot", considerSolvent=True, saveInitialData=False, runBayesCAMPD=True))
    # </editor-fold>

    # <editor-fold desc="Process Optimization using the optimal solvent">
    solvent = "C2H6O2"
    main(get_params("QH", "OneShot", considerSolvent=False, saveInitialData=True, runBayesCAMPD=False, solvent=solvent))
    time.sleep(10)
    main(get_params("QH", "Bayesian", considerSolvent=False, saveInitialData=False, runBayesCAMPD=True, solvent=solvent))
    time.sleep(10)
    main(get_params("TAC", "Bayesian", considerSolvent=False, saveInitialData=False, runBayesCAMPD=True, solvent=solvent))
    time.sleep(10)
    main(get_params("TAC", "OneShot", considerSolvent=False, saveInitialData=False, runBayesCAMPD=True, solvent=solvent))
    # </editor-fold>
