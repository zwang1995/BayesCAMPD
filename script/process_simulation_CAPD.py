# Created on 08 Jun 2022 by Zihao Wang
# Automated process simulation for the extractive distillation process with a given solvent

from utility_aspen import *


def run_simulation_CAPD(aspen_plus, solvent_list, combine_loop, txt_file=None, label=None, n_reconnect=32):
    file_bkp = aspen_plus.file_bkp
    FeedFlowrate = 500
    Output_array = []
    TotalTimeCost = 0

    solvent = solvent_list[0]
    for run_Index, Input in enumerate(combine_loop):
        ...
        if (solvent is not None) and (solvent != "C5H9NO-D2"):
            aspen_plus.Application.Tree.FindNode(r"\Data\Components\Specifications\Input\ANAME1\SOLVENT").Value = \
                solvent
...
if __name__ == "__main__":
    solvent_list = ["C2H6O2"]
    combine_loop = [
        [54.0, 1.0000006653020388, 5.21264042873379, 1.000000161496421, 10.0, 0.20000052748041633, 2.500001635123941] # QH
        # [52.0, 1.0321999539866633, 5.999999974441093, 1.0000000038295684, 8.0, 0.200000013005433, 2.5000000085592227] # TAC
    ]
    ...
    _ = run_simulation_CAPD(aspen_plus, solvent_list, combine_loop)
    time.sleep(10)
    aspen_plus.close_bkp()
